...
+ (BOOL) isValidPhone:(NSString*)value {
    ...
    NSString *string = @"\\d{3}-\\d{8}|\\d{4}-\\d{7}|\\d{4}-\\d{8}|\\d{7}|\\d{8}|\\d{10}|\\d{11}|\\d{12}|\\d{13}|\\d{14}|\\+86-\\d{7}|\\+86-\\d{8}|\\+86-\\d{10}|\\+86-\\d{11}|\\+86-\\d{12}|\\+86\\d{7}|\\+86\\d{8}|\\+86\\d{10}|\\+86\\d{11}|\\+86\\d{12}|\\+\\d{6}|\\+\\d{7}|\\+\\d{7}|\\+\\d{9}|\\+\\d{10}|\\+\\d{11}|\\+\\d{12}|\\+\\d{13}|\\+\\d{14}";
    NSPredicate *regextestcu = [NSPredicate predicateWithFormat:@"SELF MATCHES %@", string];
    
    if([regextestcu evaluateWithObject:value] == YES)
    {
        return YES;
    }
    else
...
+ (BOOL) isValidMobilePhone:(NSString*)mobile
{
    /**
     * 手机号码
...
+ (BOOL) isValid:(IdentifierType) type value:(NSString*) value
{
    ...
    switch (type)
    {
        ...
        case IdentifierTypeBirthday:
            ...
            break;
        case IdentifierTypeMobilePhone:
            result = [IdentifierValidator isValidMobilePhone:value];
        default:
            break;
    }
    return result;
