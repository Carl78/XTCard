...
@interface CardEdittingViewController ()<UIAlertViewDelegate>
...
@implementation CardEdittingViewController
...
-(id)initWithName:(NSString *)name andTargetValue:(NSString *)value{
    ...
    if (self) {
        ...
        if ([name isEqualToString:@"手机"]||[name isEqualToString:@"姓名"]||[name isEqualToString:@"公司"]) {
...
- (void)rightSideMenuButtonPressed:(id)sender {
    ...
    if(text.length==0||text==nil){
        ...
        {
            self.operationBlock(self.targetView.titleLabel.text,self.targetView.textField.text);
            [self.navigationController popViewControllerAnimated:YES];
        }
    }
    else
