...
@interface CardSearchViewController () <MLKMenuPopoverDelegate>{
...
- (void)setupOriginData{
    ...
    [cardHttpRequestService getCardWithCompangOrName:@"" pageNumber:pageNumber pageSize:kPageSize sortField:1 userId:0 hasShare:YES success:^(NSString *strToken) {
...
- (void)loadMoreData{
    ...
    [cardHttpRequestService getCardWithCompangOrName:content pageNumber:pageNumber pageSize:kPageSize sortField:sortField userId:0 hasShare:YES success:^(NSString *strToken) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (!self.isNibRegistered) {
        UINib *nib = [UINib nibWithNibName:@"CardSearchListCell" bundle:nil];
        [tableView registerNib:nib forCellReuseIdentifier:CardListCellIdentifier];
        self.isNibRegistered = YES;
    }
    
    CardSearchListCell *cardListCell = [tableView dequeueReusableCellWithIdentifier:CardListCellIdentifier];
    ...
    if (!cardListCell) {
        cardListCell = [[CardSearchListCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:CardListCellIdentifier];
    }
    ...
    cardListCell.closeValueLabel.text = [[NSString alloc]initWithFormat:@"维护人:%@",card.MainName];
...
- (void)searchBarSearchButtonClicked:(UISearchBar *)searchBar{
    ...
    cardHttpRequestService = [[CardHttpRequestService alloc]init];
    [cardHttpRequestService getCardWithCompangOrName:searchContent pageNumber:pageNumber pageSize:kPageSize sortField:sortField userId:0 hasShare:YES success:^(NSString *strToken) {
...
- (void)menuPopover:(MLKMenuPopover *)menuPopover didSelectMenuItemAtIndex:(NSInteger)selectedIndex
{
    ...
    cardHttpRequestService = [[CardHttpRequestService alloc]init];
    [cardHttpRequestService getCardWithCompangOrName:bar.text pageNumber:pageNumber pageSize:kPageSize sortField:sortField userId:0 hasShare:YES success:^(NSString *strToken) {
...
- (void)rightSideMenuButtonPressed:(id)sender {
    ...
    if (isPopup) {
        ...
        self.menuPopover = [[MLKMenuPopover alloc] initWithFrame:CGRectMake(kScreenWidth-140-60, 50, 120+60, 44*imageNames.count) menuItems:self.menuItems andImages:imageNames];
