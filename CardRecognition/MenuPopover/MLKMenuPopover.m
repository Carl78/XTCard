...
#define kMenuCellBGColor 0x403f3f //菜单单元背景颜色
...
// 新添加
-(id)initWithFrame:(CGRect)frame menuItems:(NSArray *)aMenuItems andImages:(NSArray *)images{
    ...
    if (self)
    {
        ...
        self.currentTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 11, frame.size.width, frame.size.height)];
        ...
        self.currentTableView.tag = MENU_TABLE_VIEW_TAG;
        
        UIImageView *bgView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"Menu_PopOver_BG"]];
        self.currentTableView.backgroundView = bgView;
        
        [self addSubview:self.currentTableView];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (cell == nil)
    {
        ...
        [cell setBackgroundColor:[UIColor colorWithRed:((float)((kMenuCellBGColor & 0xFF0000) >> 16))/255.0 green:((float)((kMenuCellBGColor & 0xFF00) >> 8))/255.0 blue:((float)(kMenuCellBGColor & 0xFF))/255.0 alpha:1.0]];
    }
...
-(void)changeMenuItemsName:(NSArray *)aMenuItems{
    self.menuItems = [aMenuItems copy];
